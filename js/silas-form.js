...
export function initSilasForm() {
    ...
    function showNotification(msg, type = 'info') {
        ...
        n.innerHTML = '<i class="fas fa-' + (icons[type] || icons.info) + '"></i> ' + msg;
        ...
        if (isError) { 
            ...
            card.innerHTML = `
                <h4 style="color:${errorColor};">
                    <i class="fas fa-${errorIcon}"></i> ${data.keyword}
                </h4>
                <p style="color:${errorColor};">
                    ${isRateLimitError ? 'API-Limit erreicht - ' : 'Fehler: '}${data.error}
                </p>
                ${isRateLimitError ? '<p style="color:var(--text-color-muted);font-size:0.85rem;">Tipp: Versuche es in ein paar Minuten erneut.</p>' : ''}
            `; 
        } else { 
            card.innerHTML = `
                <h4 style="color:var(--text-color);margin:0 0 10px 0;">${data.keyword}</h4>
                <p style="color:var(--text-color-muted);font-size:0.9rem;">
                    <strong style="color:var(--accent-color);">Titel:</strong> ${data.post_title || 'N/A'}
                </p>
                <p style="color:var(--text-color-muted);font-size:0.9rem;">
                    <strong style="color:var(--accent-color);">Meta:</strong> ${data.meta_description || 'N/A'}
                </p>
