...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const H1_TYPING_SPEED = 120; // ms pro Zeichen
    const H1_DELETING_SPEED = 70; // ms pro Zeichen
    const H1_DELAY_BETWEEN_TEXTS = 2200; // ms Pause nach Tippen
...
if (aiForm) {
    ...
    // Die korrigierte Start-Funktion
    const startPlaceholderAnimation = () => {
        clearInterval(typeInterval);
        clearInterval(deleteInterval);
        
        // KORREKTUR: Leert den Platzhalter vor jeder neuen Animation.
        if(aiQuestionInput) aiQuestionInput.placeholder = ""; 
